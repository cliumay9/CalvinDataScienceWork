...
xdf <- data.frame()  # xdf stands for "X dataframe"
...
# Plot Data
ggplot(xdf, aes(x = x)) + geom_histogram(binwidth = 2, fill = "#1D76B5", colour = "white") +
  xlim(10, 90) +
  xlab("Scores on Outcome Variable") +  # Title on Y axis
  ylab("Frequency") +
  theme(text = element_text(size = 24)) +
...
# AND http://www.rforge.net/doc/packages/animation/conf.int.html
...
conf <- rep(.95, k)
s  <- c(1, 2, 4, 8, 16) #Changing Standard deviaiton
llim <- rep(NA, k)
mean <- rep(NA, k)
ulim <- rep(NA, k)
