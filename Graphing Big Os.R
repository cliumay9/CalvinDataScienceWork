...
xl <- c(0, 100) #How many elements
yl <- c(0, 1500) #Operations

# Choose a palette from the RColorBrewer
...
# 2. O(log(n)): Logarithmic Ex: Find an item in a sorted array
plot(function(n) (log(n)), xlim = xl, ylim = yl, 
     col = pal[6], lwd = 2, add = TRUE)

# 3. O(n): Linear Ex: Find an item in an unsorted list
plot(function(n) (n), xlim = xl, ylim = yl, 
     col = pal[5], lwd = 2, add = TRUE)

# MODERATE
# 4. O(n log(n)): Loglinear Ex: FFT
plot(function(n) (n*log(n)), xlim = xl, ylim = yl, 
     col = pal[4], lwd = 2, add = TRUE)

# SLOW
# 5. O(n^2): Quadratic Ex: Multiply two n-digit numbers
plot(function(n) (n^2), xlim = xl, ylim = yl, 
     col = pal[3], lwd = 2, add = TRUE)

# 6. O(2^n): Exponential Ex: an exact solution traveling salesman  problem using dynamic programming
plot(function(n) (2^n), xlim = xl, ylim = yl, 
     col = pal[2], lwd = 2, add = TRUE)

# 7. O(n!): Factorial Ex: brute-force approach traveling salesman
